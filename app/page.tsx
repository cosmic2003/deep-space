...
export default async function Home({ searchParams }: PageProps) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 self-end">
      {items.map((it) => (
        <span
          key={it.code}
          className={`inline-flex items-center gap-1.5 sm:gap-2.5 rounded-full bg-zinc-900/70 px-2.5 sm:px-4 py-1.5 sm:py-2.5 ring-1 ring-inset ${it.ring}`}
        >
          <span className={`h-2 w-2 sm:h-3 sm:w-3 rounded-full ${it.dot}`} />
          <span className={`text-xs sm:text-base font-bold tracking-wide ${it.text}`}>{it.code}</span>
          <span className="text-xs sm:text-base text-zinc-200">{it.desc}</span>
        </span>
      ))}
    </div>
  );
